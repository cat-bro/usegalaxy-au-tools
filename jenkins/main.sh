...
echo '1 A pc c'
echo $(git diff $GIT_PREVIOUS_COMMIT $GIT_COMMIT --name-only --diff_filter=A  | grep "^requests\/[^\/]*$")
echo '2 D pc c'
echo $(git diff $GIT_PREVIOUS_COMMIT $GIT_COMMIT --name-only --diff_filter=D | grep "^requests\/[^\/]*$")

echo '3 A c pc'
echo $(git diff $GIT_COMMIT $GIT_PREVIOUS_COMMIT --name-only --diff_filter=A | grep "^requests\/[^\/]*$")
echo '4 D c pc'
echo $(git diff $GIT_COMMIT $GIT_PREVIOUS_COMMIT  --name-only --diff_filter=D | grep "^requests\/[^\/]*$")
