...
def main():
    ...
    names = args.names
    incautious = args.incautious

    if not names:
        raise Exception('Arguments --names (-n) must be provided.')

    uninstall_tools(galaxy_url, api_key, names, incautious)

def uninstall_tools(galaxy_server, api_key, names, incautious):
    ...
    for name in names:
        ...
        elif len(matching_tools) > 1 and not incautious:
            print(
                '*** Warning: More than one toolshed tool found for %s.  ' % name
                + 'Not uninstalling any of these tools.  Run script with --incautious (-i) flag to uninstall anyway'
            )
        else:  # Either there is only one matching tool for the name and revision, or there are many and incautious=True
